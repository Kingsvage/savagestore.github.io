...
exports.getTopupCatalog = onCall({ secrets: [fazercardsKey] }, async (request) => {
  ...
  const settings = (await db.doc("settings/private").get()).data() || {};
...
const LISTING_STATUSES = ["pending-review", "approved", "rejected", "suspended", "sold", "verification-failed"];
...
function marketplaceAmounts(finalSalePrice) {
  const finalPrice = safeNumber(finalSalePrice, "finalSalePrice");
  const marketplaceFee = Math.round(finalPrice * MARKETPLACE_FEE_RATE);
  return { finalSalePrice: finalPrice, marketplaceFeeRate: MARKETPLACE_FEE_RATE, marketplaceFee, sellerPayout: finalPrice - marketplaceFee };
}

exports.submitMarketplaceListing = onCall(async (request) => {
  const sellerId = requireAuth(request);
  const publicData = request.data?.public || {};
  const secretData = request.data?.secret || {};
  if (publicData.sellerAgreementAccepted !== true) throw new HttpsError("failed-precondition", "Seller agreement acceptance is required.");
  const required = ["game", "title", "region", "price", "level", "rank", "description"];
  if (required.some((key) => !publicData[key])) throw new HttpsError("invalid-argument", "Complete all required listing fields.");
  if (!cleanText(secretData.loginMethod, 60) || !cleanText(secretData.loginIdentifier, 200) || !cleanText(secretData.password, 500)) {
    throw new HttpsError("invalid-argument", "Private verification details are required.");
  }
  const seller = await admin.auth().getUser(sellerId);
  const listingRef = db.collection("listings").doc();
  const now = admin.firestore.FieldValue.serverTimestamp();
  await db.runTransaction(async (tx) => {
    tx.set(listingRef, {
      sellerId, sellerName: seller.displayName || "Seller", sellerEmail: seller.email || "",
      game: cleanText(publicData.game, 40), title: cleanText(publicData.title, 140), region: cleanText(publicData.region, 80),
      price: safeNumber(publicData.price, "price"), level: cleanText(publicData.level, 80), rank: cleanText(publicData.rank, 100),
      description: cleanText(publicData.description, 2000), rareItems: cleanText(publicData.rareItems, 1000), accountUid: cleanText(publicData.accountUid, 100),
      image1: cleanText(publicData.image1, 2000), image2: cleanText(publicData.image2, 2000), image3: cleanText(publicData.image3, 2000),
      status: "pending-review", sellerAgreementAccepted: true, sellerAgreementVersion: "2026-09", sellerAgreementAcceptedAt: now, createdAt: now
    });
    tx.set(db.doc(`listingSecrets/${listingRef.id}`), {
      listingId: listingRef.id, loginMethod: cleanText(secretData.loginMethod, 60), loginIdentifier: cleanText(secretData.loginIdentifier, 200),
      password: cleanText(secretData.password, 500), verificationInfo: cleanText(secretData.verificationInfo, 2000), createdAt: now, updatedAt: now
    });
  });
  return { listingId: listingRef.id, status: "pending-review" };
});

exports.getListingCredentialsForAdmin = onCall(async (request) => {
  const adminId = requireAuth(request); await requireAdmin(adminId);
  const listingId = cleanText(request.data?.listingId, 200);
  const secret = await db.doc(`listingSecrets/${listingId}`).get();
  if (!secret.exists) throw new HttpsError("not-found", "Verification details not found.");
  await db.collection("credentialAccessLogs").add({ adminId, listingId, action: "ADMIN_VIEWED_LISTING_CREDENTIALS", timestamp: admin.firestore.FieldValue.serverTimestamp() });
  return secret.data();
});

exports.createAccountSale = onCall(async (request) => {
  const buyerId = requireAuth(request);
  const listingId = cleanText(request.data?.listingId, 200);
  const listingSnap = await db.doc(`listings/${listingId}`).get();
  if (!listingSnap.exists || listingSnap.data().status !== "approved") throw new HttpsError("failed-precondition", "This listing is unavailable.");
  const listing = listingSnap.data(); const buyer = await admin.auth().getUser(buyerId);
  const saleRef = db.collection("accountSales").doc();
  await saleRef.set({ saleId: saleRef.id, listingId, sellerId: listing.sellerId, buyerId, sellerEmail: listing.sellerEmail || "", buyerEmail: buyer.email || "", game: listing.game || "", listingTitle: listing.title, originalListingPrice: listing.price, paymentStatus: "pending", transferStatus: "pending", technicalSupportStatus: "requested", createdAt: admin.firestore.FieldValue.serverTimestamp(), updatedAt: admin.firestore.FieldValue.serverTimestamp() });
  return { saleId: saleRef.id };
});

exports.updateAccountSaleByAdmin = onCall(async (request) => {
  const adminId = requireAuth(request); await requireAdmin(adminId);
  const saleId = cleanText(request.data?.saleId, 200); const patch = request.data?.patch || {};
  const saleRef = db.doc(`accountSales/${saleId}`); const saleSnap = await saleRef.get();
  if (!saleSnap.exists) throw new HttpsError("not-found", "Sale not found.");
  const updates = { adminId, updatedAt: admin.firestore.FieldValue.serverTimestamp() };
  if (patch.finalSalePrice !== undefined) Object.assign(updates, marketplaceAmounts(patch.finalSalePrice));
  ["paymentStatus", "transferStatus", "technicalSupportStatus", "notes", "paymentMethod", "paymentReference"].forEach((key) => { if (patch[key] !== undefined) updates[key] = cleanText(patch[key], 2000); });
  if (updates.paymentStatus === "payment-confirmed") updates.confirmedAt = admin.firestore.FieldValue.serverTimestamp();
  if (updates.transferStatus === "completed") {
    updates.completedAt = admin.firestore.FieldValue.serverTimestamp();
    const sale = saleSnap.data();
    await db.doc(`listings/${sale.listingId}`).update({ status: "sold", soldAt: admin.firestore.FieldValue.serverTimestamp(), soldBySaleId: saleId });
    const amounts = updates.finalSalePrice ? updates : sale;
    await db.collection("sellerSettlements").doc(saleId).set({ sellerId: sale.sellerId, saleId, listingId: sale.listingId, ...amounts, payoutStatus: "pending", createdAt: admin.firestore.FieldValue.serverTimestamp(), processedBy: adminId }, { merge: true });
  }
  await saleRef.update(updates); return { saleId, ...updates };
});
